...
<head> 
	...
	<script type="text/javascript" src="<%=path%>/js/common/bootstrap-datepicker.js"></script>
...
<body>
	<h3>任务办理—[${hasFormKey ? task.name : data.task.name}]，流程定义ID：[${hasFormKey ? task.processDefinitionId : data.task.processDefinitionId}]</h3>
	<hr/>
	<form action="<%=path%>/task/complete/${hasFormKey ? task.id : data.task.id}" class="form-horizontal" method="post">
